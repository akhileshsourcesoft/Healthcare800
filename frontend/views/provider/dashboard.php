...
<div class="container">
	<div class="row" style="padding:15px 0 30px 0">
		<div class="contact-providers-form">
		 <a href="<?php echo Yii::$app->getUrlManager()->createUrl('provider/updateprofilestep1');?>"><button  name="Edit Profile" class="btn btn-success">Complete Your Profile</button></a>
		  <a href="<?php echo Yii::$app->getUrlManager()->createUrl('provider/viewappointment');?>"><button  name="View Appointment" class="btn btn-success">View Appointment</button></a>
		  ...
		  <fieldset>
			...
			<div class="time_slot">
				...
				if(!$model->isNewRecord){
				  foreach($daylistModel as $key=>$dayvalue){
					// $providerDayAvail = ProvidersDayAvailability::find()->where(['provider_id'=>$model->id])->andWhere(['day_id'=>$dayvalue['id']])->one();
					// $query = new Query;
					// $query->select('*')->from('hc_providers_time_availability')->where(['day_availability_id'=>$providerDayAvail['id']])->all();
					// $providerTimeAvail = $query->createCommand()->queryAll();
					// $start_time = array(); $timeAvailId = array();
				 ...
				 ?>
					<ul>
						<li><?php echo $dayvalue['day_name'];?></li>
						<li>Morning Time
							<ul class="timeTable">
							<?php
								switch($dayvalue['id']){
							...
							$providerDayAvail = ProvidersDayAvailability::find()->where(['provider_id'=>$model->id])->andWhere(['day_id'=>$dayvalue['id']])->one();
							 ...
							 for($i=9; $i<=11; $i++){
								...
								//$query->select('*')->from('hc_providers_time_availability')->where(['day_availability_id'=>$providerDayAvail['id']])	
								//$query = $query->createCommand()->queryOne();
								
								$modelsql = $connection->createCommand("SELECT * FROM hc_providers_time_availability where day_availability_id = '".$providerDayAvail['id']."'");
								$usersdata = $modelsql->queryOne();
								
								//print_r($usersdata);
								
								$providersTimes = ProvidersTimeAvailability::find()->where(['day_availability_id'=>$providerDayAvail['id']])->one();
								 if($usersdata){
								?>
								<li style="background-color:red !important; color:#fff;" class="<?php echo $checked.'_'.$dayvalue['id'].' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$dayvalue['id'];?>" onclick="providerupAvail(this.id,'<?php echo $dayvalue['id'];?>','A','M')"><?php echo $usersdata['start_time'];?>
								<?php }else{ echo 'el';?>
								<li class="<?php echo $checked.'_'.$dayvalue['id'].' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$dayvalue['id'];?>" onclick="providerAvail(this.id,'<?php echo $dayvalue['id'];?>','M')"><?php echo $intValue;?>
							 <?php  } 
							      }
								} 
							?>
						...
						<li>After Noon Time
							...
							<?php
							 ...
							 ?>
								<li class="<?php echo $checked.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$dayvalue['id'];?>" onclick="providerAvail(this.id,'<?php echo $dayvalue['id'];?>','A')"><?php echo $intValue;?></li>
							<?php } 
							}
						?>
						...
						<ul class="timeTable">
							<?php
							 ...
							 ?>
								<li class="<?php echo $checked.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$dayvalue['id'];?>" onclick="providerAvail(this.id,'<?php echo $dayvalue['id'];?>','E')"><?php echo $intValue;?></li>
...
<script>
function providerAvail(elementId, slotday, slotTime){
	var slotTimeval = $('#'+elementId).text();
	var slotTime = slotTime;
	var slotday = slotday;
	var attrId = elementId.split("_");
	if($('#'+elementId).hasClass('checkedTimeslot_'+slotday)){
		$('#'+elementId).addClass('uncheckedTimeslot_'+slotday).removeClass('checkedTimeslot_'+slotday).removeAttr('style');
		$('#slotTime_'+attrId[1]+'_'+attrId[2]+'_'+attrId[3]).remove();
		$('#slots_'+attrId[1]+'_'+attrId[2]+'_'+attrId[3]).remove();
		
	}else{
		$('#'+elementId).addClass('checkedTimeslot_'+slotday).removeClass('uncheckedTimeslot_'+slotday).css({"background":"red", "color":"#fff"});
		$('#'+elementId).after("<input id='slotTime_"+attrId[1]+'_'+attrId[2]+'_'+attrId[3]+"' class='slotTime' type='hidden' name='slotTime["+slotday+"][]' value='"+slotTimeval+"'><input id='slots_"+attrId[1]+'_'+attrId[2]+'_'+attrId[3]+"' class='slots' type='hidden' name='slots[]' value='"+slotTime+"'>");
	}
