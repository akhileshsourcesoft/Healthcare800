				...
				<?php 
                  if(count($userListdata)>0){  
					 ...
					 foreach($locationdata as $locationVal){ 
						 $userCitylist = User::find()->select('id')->where(['city'=>$locationVal['city'], 'status'=>1])->asArray()->all();
						 $locationsId = '';
						 $lcount=1;
          ...
          <ul class="fliterResult">
            ...
            <li>
              <aside class="col-md-7">
                ...
                <div class="feedBack"><a href="<?php echo Yii::$app->getUrlManager()->createUrl("provider/detail?pid=".$val['password_reset_token']."");?>"><img src="<?php echo Yii::$app->getUrlManager()->createUrl('images/feedback.png');?>">12 Feedback</a></div>
              </aside>
								...
								?>
                                  <tr>
                                    ...
                                    <td>
                                    	<ul class="table_inside_datat">
											...
											foreach($providerVal['providersTimeAvailabilities'] as $key=>$timeSlotvalue){ 
												...
												<?php }else{ ?>
													<li><a href="<?php echo Yii::$app->getUrlManager()->createUrl("provider/detail?pid=".$val['password_reset_token']."&date=".$providerVal['slot_date']."&btime=".$timeSlotvalue['start_time']."");?>"><?php echo $timeSlotvalue['start_time'];?> AM</a></li>
												<?php  }
												} 
										    $mcounter++;}
											?>
                                        	...
                                        	foreach($providerVal['providersTimeAvailabilities'] as $key=>$timeSlotvalue){ 
												if($timeSlotvalue['start_time'] >= '12:00' && $timeSlotvalue['start_time'] <= '13:50'){ 
												 if($timeSlotvalue['start_time']<=$current_time && $providerVal['slot_date']==$currentDate){ ?>
													...
													<?php }else{ ?>
														<li><a href="<?php echo Yii::$app->getUrlManager()->createUrl("provider/detail?pid=".$val['password_reset_token']."&date=".$providerVal['slot_date']."&btime=".$timeSlotvalue['start_time']."");?>"><?php echo $timeSlotvalue['start_time'];?> PM</a></li>
													<?php  }
													 } 
										    	$acounter++;} 
											?>
                                    ...
                                    <td><ul class="table_inside_datat">
                                        	 <?php 
											$timeslotbooking = UserTimeslotBooking::find()->where(['booking_date'=>$providerVal['slot_date']])->andWhere(['provider_id'=>$val['id']])->asArray()->all();
											$booking_time[] = '';
											foreach($timeslotbooking as $bkey=>$bookingval){
												$booking_time[] = $bookingval['booking_time'];
											}
                                        	$ecounter=1;
                                        	foreach($providerVal['providersTimeAvailabilities'] as $key=>$timeSlotvalue){ 
												if($timeSlotvalue['start_time'] >= '18:00' && $timeSlotvalue['start_time'] < '20:00'){ 
													if($timeSlotvalue['start_time']<=$current_time && $providerVal['slot_date']==$currentDate){ ?>
														<li><span><?php echo $timeSlotvalue['start_time'];?> PM</span></li>
													<?php }else{ ?>
														<?php if(!in_array($timeSlotvalue['start_time'], $booking_time)){ ?>
															<li><a href="<?php echo Yii::$app->getUrlManager()->createUrl("provider/detail?pid=".$val['password_reset_token']."&date=".$providerVal['slot_date']."&btime=".$timeSlotvalue['start_time']."");?>"><?php echo $timeSlotvalue['start_time'];?> PM</a></li>
														<?php }else{ ?>
															<li><span><?php echo $timeSlotvalue['start_time'];?> PM</span></li>
												     <?php	}
...
$(document).ready(function(){
	...
	$("#filterButton").click(function(){
		...
		if(parseInt(convertTo24Hour(mintime)) > parseInt(convertTo24Hour(maxtime))){
