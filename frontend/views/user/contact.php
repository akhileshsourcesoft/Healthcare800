...
use common\models\OptionContent;
...
$qualificationlist = array_merge($qualificationlistModel, array('Other'=>'Others'));
...
<!-- featured Panels -->
	...
	<!------------------------------ Product Content -------------------------------->	   
		<div id="tab1" class="tab-pane fade in active">
			...
			<?= $form->field($model, 'fname')->textInput(['maxlength' => true, 'id'=>'user-fnames'])->label('First Name');?>
				<span id="user_fname_msg"></span>
			<?= $form->field($model, 'lname')->textInput(['maxlength' => true, 'id'=>'user-lnames'])->label('Last Name');?>
				<span id="user_lname_msg"></span>
			<?= $form->field($model, 'email')->textInput(['maxlength' => true, 'id'=>'user-emails']) ?>
			...
			<?= $form->field($model, 'gender')->radioList(['1' => 'Male', '0' => 'Female']);?>
			 ...
			 <?= $form->field($model, 'qualification_id')->dropDownList($qualificationlist, ['prompt'=>'Select Qualification / Degree'])->label('Qualification / Degree'); ?>
					...
					<ul>
						...
						<li>Morning Time
							...
							<?php
							 ...
							 ?>
								<li class="uncheckedTimeslot_<?php echo $m.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$m;?>" onclick="providerAvail(this.id,'<?php echo $m;?>','M')"><?php echo $intValue;?></li>
							<?php } 
							}
						?>
						...
						<li>After Noon Time
							...
							<?php
							 ...
							 ?>
								<li class="uncheckedTimeslot_<?php echo $m.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$m;?>" onclick="providerAvail(this.id,'<?php echo $m;?>','A')"><?php echo $intValue;?></li>
							<?php } 
							}
						?>
						...
						<ul class="timeTable">
							<?php
							 ...
							 ?>
								<li class="uncheckedTimeslot_<?php echo $m.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$m;?>" onclick="providerAvail(this.id,'<?php echo $m;?>','E')"><?php echo $intValue;?></li>
							<?php } 
							}
						?>
						...
						<ul class="timeTable">
							<?php
							 ...
							 ?>
								<li class="uncheckedTimeslot_<?php echo $m.' '.$className?>" id="<?php echo $className.'_'.$i.'_'.$j.'_'.$m;?>" onclick="providerAvail(this.id,'<?php echo $m;?>','N')"><?php echo $intValue;?></li>
							<?php } 
							}
						?>
		...
		<div class="form-group">
			<?= Html::Button($model->isNewRecord ? 'Register' : 'Update', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary','onclick' => 'validateContactForm()']) ?>
...
function validateContactForm(){
	...
	if(user_qualification_id.length == 0 || user_qualification_id.val() == ""){
		...
		$("#user_qualification_id_msg").empty();
	}
	
	if(user_qualification_id.val()=='Other'){
		if($("#qualification-other_qname").val()==''){
			$("#qual_other_msg").text("Please enter other.").css('color','red');
			valid = 0;
		}else{
			$("#qual_other_msg").empty();
		}
	}
	...
	if(user_city.val() == ""){
	 ...
	 var isChecked = jQuery("input[name='User[gender]']:checked");
     if(isChecked.length==0){
         $("#user_gender_msg").text("Please checked atleast one.").css('color','red');
         valid = 0;
     }else{
		$("#user_gender_msg").empty();
	}
