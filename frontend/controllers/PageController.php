    ...
    public function behaviors()
{
    ...
    { 
      ...
      if (!is_null($model)) {
		  ...
		  if(isset($_POST) && !empty($_POST)){ 
			   $contactmodel->attributes = $_POST['Contactus']; 
			  $userdata = Yii::$app->request->post('Contactus');		  
  ...
  public function actionSlug($slug)
    { 
	  //$formmodel = new Forms();
	  $contactmodel = new Contactus();
	  $contactprovidermodel = new Contactprovider();
      ...
      if (!is_null($model)) { 
		if(isset($_POST) && !empty($_POST)){
			...
			<h3 style="color:#f04e4e; text-transform:uppercase;" align="center">Thank you, Healthcare800</h3><br/><br/>
				...
				->setTo($emailsend)
				->setFrom($emaildata[0])
				->setSubject('Contact us Query from HealthCare800.')
				->setHtmlBody($html)
				->send();
           ...
           return $this->render('view', [
              'model' => $model, 'contactmodel'=>$contactmodel, 'contactprovidermodel'=>$contactprovidermodel,
          ]); 
