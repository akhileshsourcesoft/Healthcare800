...
class UserController extends Controller
{
      ...
      $model = new User();
        if($model->load(Yii::$app->request->post())){ 
			...
			if(count($checkEmailid)==0){
					...
					<p>Kindly confirm your email.</p><p><a href="'.Yii::$app->request->hostInfo."/YAB01/user/userconfirm?id=".$id.'">'.Yii::$app->request->hostInfo."/YAB01/user/userconfirm?id=".$id.'</a></p><p>Your Password: '.$password.'</p><p><b>Thanks & Regards,</b><br>Healthcare800 Team,<br><a href="http://www.healthcare800.us" target="_blank">www.healthcare800.us</a></p>';
					$messageuser="$url";
   ...
   public function actionDashboard(){ 
	...
	public function actionSearchtext(){
		$searchtxt = trim(urldecode($_POST['searchtxt']));
			
		if(strpos($searchtxt, " ") !== false){		
		$name   = explode(' ',$searchtxt);
       ...
       $category_id = $_POST['category_id'];
		$query = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')
              ...
              {
				$str='OR (hc_users.fname ="' . $name[0]. '" '.' AND hc_users.lname = "' . $name[1] . '")';
		      }
              $query->andWhere('hc_users.fname LIKE "%'.$searchtxt.'%" '.' OR hc_users. LIKE "%'.$searchtxt.'%" '.$str);
...
public function actionLoadmoreproduct(){ 

		isset($_POST["page"]) && $_POST["page"] != '' ? $page_number = $_POST["page"] : $page_number = 0;
		isset($_POST["product_per_page"]) && $_POST["product_per_page"] != '' ? $product_per_page = $_POST["product_per_page"] : $product_per_page = Yii::$app->params['product_per_page'];
		
		isset($_POST['sort_order']) && $_POST['sort_order']!="" ? $sort_order = $_POST['sort_order'] : $sort_order = "";
		$position = ($page_number * $product_per_page);
		
		$categoryId = Categories::find()->where(['slug'=>$category_slug])->one();
		
		$productData = ProductCategories::find()->joinWith('product')->where(['bs_product.status' => 1, 'bs_product_categories.category_id'=>$categoryId->category_id])->andWhere('bs_product.date_available = 0000-00-00 OR bs_product.date_available <= CURDATE()');
		
		if(!empty($minprice) || $minprice != 0) {
			$productData = $productData->andWhere(['>=', 'CASE WHEN `bs_product`.`sale_price` != 0.00 THEN `bs_product`.`sale_price` ELSE `bs_product`.`price` END',$minprice]);
		}
		if(!empty($maxprice) || $maxprice != 0) {
			$productData = $productData->andWhere(['<=', 'CASE WHEN `bs_product`.`sale_price` != 0.00 THEN `bs_product`.`sale_price` ELSE `bs_product`.`price` END',$maxprice]);
		}	
		#---Sort By---#
		if($sort_order =='id'){
			$productData = $productData->orderBy(['bs_product.product_id'=>SORT_DESC]);
		}else if($sort_order == 'name'){
			$productData->orderBy(['bs_product.name'=>SORT_ASC]);
		}else{		
			 $productData = $productData->orderBy(['bs_product.sort_order'=>SORT_DESC]);
		}
		#---End Sort By---#
		#---Show Per Page---#
		/*if(!empty($product_per_page)){
			$countProduct = $productData->count();
			$total_pages = ceil($countProduct/$product_per_page);				
		}*/
		$productData = $productData->offset($position)->limit($product_per_page)->asArray()->all();	
     
		#---End Show Per Page---#
		
		if(is_array($productData) && !empty($productData)){
			foreach($productData as $k=> $p){
				$img = explode('/',$p['product']['image']);
				Image::getImagine()->open(Yii::getAlias('@webroot/uploads/'.$p['product']['image']))->thumbnail(new Box(124, 130))->save(Yii::getAlias('@runtime/thumbs/124X130-'.$img[1]) , ['quality' => 90]);
			}
		}
