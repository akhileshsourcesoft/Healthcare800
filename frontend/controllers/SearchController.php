...
$userListdata1 = User::find()->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->where('hc_providers_day_availability.slot_date >= CURDATE()')->andWhere(['hc_users.id'=>$pid])
						->asArray()->all();



//$userListdata1= ProvidersDayAvailabilities::find()->where(['hc_providers_day_availability.provider_id'=>$pid])->asArray()->all();;

//print_r($userListdata1); 


return $this->render('listingnew',['userListnewdata' => $userListdata1]); die;
	}
   ...
   public function actionListing(){
	 ...
	 if(!empty($_GET['category']) && (!empty($_GET['pname']))){
		...
		//$firstname = @$pname[0];
		//$lastname = @$pname[1];
                 $firstname=trim(Yii::$app->request->get('pname'));
		 $lastname=trim(Yii::$app->request->get('lname'));

		$daylist = Dayname::find()->where(['status'=>1])->asArray()->all();
		$userListdata = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->joinWith('providerUserPrices')->joinWith('providerUserPrices.providerFees')
						//->where('hc_providers_day_availability.slot_date = CURDATE() OR hc_providers_day_availability.slot_date >= CURDATE()')
						->Where(['hc_users.user_role_id'=>'4'])
		...
		$locationdata = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->joinWith('providerUserPrices')->joinWith('providerUserPrices.providerFees')
						->where('hc_providers_day_availability.slot_date = CURDATE() OR hc_providers_day_availability.slot_date >= CURDATE()')
						->andWhere(['hc_users.status'=>'1'])
			...
			$query = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->joinWith('providerUserPrices')->joinWith('providerUserPrices.providerFees')->joinWith('providerInsuranceCompanies.insuranceCompanies')
						...
						if(isset($userInsurance) && (!empty($userInsurance))){
							$insuranceCompanyId = array();
							$insuranceCompany = ProviderInsuranceCompany::find()->joinWith('insuranceCompanies')->where(['hc_insurance_companies.name'=>$userInsurance])->groupBy(['hc_provider_insurance_company.provider_id'])->all();
							foreach($insuranceCompany as $insValue){
								$insuranceCompanyId[] = $insValue['insurance_companies_id']; 
							}
							$query = $query->andWhere(['IN', 'hc_provider_insurance_company.insurance_companies_id', $insuranceCompanyId]);
						}
		...
		$locationdata = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->joinWith('providerUserPrices')->joinWith('providerUserPrices.providerFees')
				...
				->asArray()->all();
			  	...
			  	return $this->render('listing', ['userListdata' => $userListdata,'userSpeciallity'=>$userSpeciallity, 'userInsurance'=>$userInsurance,'userCity'=>$userCity, 'userState'=>$userState, 'locationdata'=>$locationdata, 'daylist'=>$daylist]); 
	  }
   }
   ...
   public function actionProviderslisting(){
		...
		if(!empty($minvalue) && !empty($maxvalue)){
			$providerFeesid = ProviderFees::find()->where(['>=','hc_provider_fees.fees', $minvalue])->andWhere(['<=','hc_provider_fees.fees', $maxvalue])->all();
			$feesArrayvalue = array();
			foreach($providerFeesid as $feesvalue){
				$feesArrayvalue[] = $feesvalue['id'];
			}
			$userListdata = $userListdata->andWhere(['IN','hc_provider_user_price.provider_fees_id', $feesArrayvalue]);
		} 
	   ...
	   if(count($userListdata)>0){    
		...
		if(!empty($val['providerUserPrices'][0]['providerFees']['fees'])){
		  $providerFees = number_format($val['providerUserPrices'][1]['providerFees']['fees'],2).'&nbsp;&nbsp;-&nbsp;&nbsp;'.number_format($val['providerUserPrices'][0]['providerFees']['fees'],2);
		}	
	 $userlistdata .= '<li>
	  ...
	  <aside class="col-md-5">
		<div class="appointMent">
		  ...
		  <p><i aria-hidden="true" class="fa fa-dollar"></i>'.$providerFees.'</p>';
