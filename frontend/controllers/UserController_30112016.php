			   ...
			   $userdata = array(
                ...
                if(isset($data) && !empty($data)){
					if(isset($data->user_role_id) && (!empty($data->user_role_id))) {
						$checktrue  = json_encode(array('login'=>$data->user_role_id));
					}
                    return $checktrue;
				}else{               
      ...
      $model = new User();
        if($model->load(Yii::$app->request->post())){ 
			...
			if(count($checkEmailid)==0){
					...
					$url='Hi <b>'. $model->fname .' '.$model->lname .',</b><p>You have been registered with <b>Brightseathealth.</b></p>
					<p>Kindly confirm your email.</p><p><a href="'.Yii::$app->request->hostInfo."/YAB01/user/userconfirm?id=".$id.'">'.Yii::$app->request->hostInfo."/YAB01/user/userconfirm?id=".$id.'</a></p><p><b>Thanks & Regards,</b><br>Brightseathealth Team,<br><a href="http://www.brightseathealth.com" target="_blank">www.brightseathealth.com</a></p>';
					$messageuser="$url";
