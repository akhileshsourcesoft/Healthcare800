...
use common\models\AppointmentReasons;
use yii\db\Query;
...
class ProviderController extends Controller
{
	...
	public function behaviors()
    {
          ...
          'access' => [
                ...
                'only' => ['thankyou','register','confirm','dashboard','logout','updateprofilestep1','updateprofilestep2','updateprofilestep3','providerresetpassword','detail','newslettersubscriber','statelist', 'insurancecomplisting','bluepaypayment','providerresetpasswordstep4','filedownloads','viewappointment','shifttimeslotappointment', 'patientdetails', 'shiftproviderappointment', 'changeappointmentstatus', 'shiftappointment'],
                'rules' => [
                     [
                        'actions' => ['dashboard','logout','providerresetpassword','updateprofilestep1','updateprofilestep2','updateprofilestep3','updateprofilestep4','providerresetpasswordstep5','detail','insurancecomplisting','bluepaypayment','filedownloads','viewappointment','shifttimeslotappointment', 'patientdetails', 'shiftproviderappointment', 'changeappointmentstatus', 'shiftappointment'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
...
public function actionRegister()
	{
		...
		//	echo $_POST['User']['passwordhash']; 
		Yii::$app->session->set('userpassword', $_POST['User']['passwordhash']); 
              
			$passwordresethash=$_POST['User']['passwordhash'];
	...
	public function actionRegisterstep3(){
		...
		$healthfacilityModel = new HealthFacility();
		$model = $this->findProviderModel($ptokenid); 
		$getpassword=Yii::$app->session->get('userpassword');
       // echo $passwordresethash; die; 
		...
		if(Yii::$app->request->post()!=''){	
			...
			if(!empty($_POST['healthfacilityaddress'])){
				...
				$messageBody = '<table width="55%" border="0" cellpadding="5" cellspacing="1" align="center">
					...
					<tr><td width="25%">Password:</td><td>'.$getpassword.'</td></tr>
					...
					<tr><td colspan="2"><strong>Thanks & Regards,<br/>HealthCare800 Team<br/><a href="http://www.healthcare800.com" target="_blank">www.healthcare800.us</a></strong></td></tr>
				</table>'; 

				$providersmail = \Yii::$app->mailer->compose()
					->setTo('akhilesh.kumar@sourcesoftsolutions.com')
				...
				<p>You will get the activation link in your registered email address soon.</p><p><b>Thanks & Regards,</b><br>HealthCare800 Team,<br><a href="http://www.healthcare800.us" target="_blank">www.healthcare800.us</a></p>';
				$messageuser="$url";
	...
	public function actionDetail(){
		...
		$modeltimeslotbooking = new UserTimeslotBooking();
 		...
 		if($modeltimeslotbooking->load(Yii::$app->request->post())){
				...
				@$modeltimeslotbooking->booking_pay_type  = Yii::$app->request->post('UserTimeslotBooking')['booking_pay_type'];		$modeltimeslotbooking->user_id = Yii::$app->user->id;
				...
				if($modeltimeslotbooking->booking_pay_type!='N'){
			    ...
			    if($modeltimeslotbooking->save(false)){
				  ...
				  }else if($modeltimeslotbooking->booking_pay_type=='N'){
					$this->redirect(\Yii::$app->urlManager->createUrl("thanks/paymentprocess?bookingid=".$modeltimeslotbooking->booking_number."&pay=N"));
				 }else{
					 $this->redirect(\Yii::$app->urlManager->createUrl("thanks/paymentprocess?bookingid=".$modeltimeslotbooking->booking_number."&pay=I"));
				 }			  
			  }
		}

		if(isset($providerid) && (!empty($providerid))){
			$providerlist = User::find()->joinWith('servicesCategory')->joinWith('state')->joinWith('qualification')->joinWith('providersDayAvailabilities')->joinWith('providersDayAvailabilities.providersTimeAvailabilities')->joinWith('providerUserPrices')->joinWith('providerUserPrices.providerFees')->joinWith('providerInsuranceCompanies')->joinWith('providerContracts')
	...
	public function actionPaypalpayment(){
		...
		$form = '';
		$form .= '<html><title>www.healthcare800.us</title><head></head><body><div style="text-align:center; margin-top:30px; font-size:25px;"><p>You are being redirected to the paypal. Please don`t refresh the page.</p><img src="'.Yii::$app->homeUrl.'frontend/web/images/loader.gif"></div><form name="payment_method_form" id="payment_method_form" action=" '.$action.' " method="post">';
	...
	public function actionConfirm(){
	 ...
	 public function actionDashboard(){ 
			...
			$userData = $model;
			$daylistModel = Dayname::find()->select('day_name, id')->where(['status'=>1])->all();
			...
			if(Yii::$app->request->post()!=null){
				...
				#---- ADDED THE CALENDER DATA----#
				if(isset($_POST['slotTime']) && (!empty($_POST['slotTime']))){
					foreach($_POST['slotTime'] as $daykey=>$timevalue){
						$dayAvailability = new ProvidersDayAvailability();
						$dayAvailability->day_id = $daykey;
						$dayAvailability->provider_id = $model->id;
						$dayAvailability->save(false);
							foreach($timevalue as $slotkey=>$timeslot){
								$timeAvailability = new ProvidersTimeAvailability();
								$timeAvailability->day_availability_id = $dayAvailability->id;		 
								$timeAvailability->start_time =	$timeslot;
								$timeAvailability->slotTime = $_POST['slots'][$slotkey];
	...
	public function actionNewslettersubscriber(){
		...
		if($subscriberList==0){
			if($model->save(false)){
				...
				<p><b>Thanks & Regards,</b><br>HealthCare800 Team,<br><a href="http://www.healthcare800.us" target="_blank">www.healthcare800.us</a></p>';
				$messageBody="$url";
	...
	public function actionDeletecontract(){
		...
		$model->deleteAll(['id'=>$provideid]);
    
       Yii::$app->getSession()->setFlash('success','Contarct image is deleted.');			
	   return $this->redirect(['updateprofilestep4']);
	...
	public function actionViewappointment(){
		$id = Yii::$app->user->identity->id;
		$startdate = date( 'Y-m-d', strtotime( 'sunday previous week' ) );
		$enddate = date( 'Y-m-d', strtotime( 'saturday this week' ) );

		$timeslotbooking = UserTimeslotBooking::find()->where(['between', 'booking_date', $startdate, $enddate])->andWhere(['provider_id'=>$id])->asArray()->all();
		return $this->render('viewproviders', [
			'model' => $this->findModel($id), 'timeslotbooking'=>$timeslotbooking,
		]);
	}
    ...
    public function actionPatientdetails(){
		...
		if(!empty($bookingid)){
			$userTimeslotlist = UserTimeslotBooking::find()->with('provider', 'user', 'bookingStatus', 'paymentHistories', 'insuranceCompanies')->where(['id'=>$bookingid])->andWhere(['status'=>'1'])->one();
			if(!empty($userTimeslotlist->provider->profile_image)){
				$providerImage = $userTimeslotlist->provider->profile_image;
			}else{
				$providerImage = '/users/providers/default.png';
			}
			if($userTimeslotlist->user_type_id==1){
				$patientType = 'New Patient';
			}else{
				$patientType = 'Existing Patient';
			}
			if($userTimeslotlist->user->gender==1){
				$genger = 'Male';
			}else{
				$genger = 'Female';
			}
			...
			$str = '<div class="col-md-6 providerArea">
						<h4 style="font-size:17px;" class="slottimedate">'.date("D, M d", strtotime($userTimeslotlist->booking_date)).' - '.date('G:i A', strtotime($userTimeslotlist->booking_time)).' </h4>
						<div class="col-md-3">
							<img src="'.Yii::$app->geturlManager()->createUrl(['uploads/'.$providerImage]).'" width="50" height="60">
						</div>
						<div class="col-md-9">
							<div class="col-md-12">
								<i class="fa fa-user" aria-hidden="true"></i> '.$userTimeslotlist->provider->fname.' '.$userTimeslotlist->provider->lname.', '.$userTimeslotlist->provider->qualification->name.' 
							</div>
							<div class="clear"></div>
							<div class="col-md-12">
								<i class="fa fa-map-marker" aria-hidden="true"></i> '.$userTimeslotlist->provider->city.' 
							</div>
						</div>
						<div class="col-md-12">
							<select name="order_status" id="order_status" onchange="shiftAppointment(this.value, '.$userTimeslotlist->id.', '.$userTimeslotlist->provider->id.')">';
							$bookingStatuslist = BookingStatus::find()->where(['status'=>'1'])->andWhere(['<>', 'id', 2])->andWhere(['<>', 'id', 6])->asArray()->all();
								if(count($bookingStatuslist)>0){
										$str .= '<option value="">--Select Status--</option>';
									foreach($bookingStatuslist as $value){
										$value['id']==$userTimeslotlist->booking_status_id? $selected = 'selected="selected"': $selected = '';
					...
					<div class="col-md-6 patientArea">
						<ul>
							<li>'.$userTimeslotlist->fullname.' ('.$patientType.')</li>
							<li>'.$genger.'</li>
							<li>'.$userTimeslotlist->phone_no.'</li>
							<li><a href="mailto:'.$userTimeslotlist->email.'">'.$userTimeslotlist->email.'</a></li>';
							if($userTimeslotlist->payment_method==1){
								$insuranceComp = InsuranceCompanies::find()->where(['id'=>$userTimeslotlist->insurance_companies_id])->one();
								$str .= '<li>Insurance Type : <span>Insurance</span></li>';
								if(!empty($insuranceComp->name)){ 
									$str .= '<li>Name of the insurance : <span>'.$insuranceComp->name.'</span></li>';
								}else{
									$str .= '<li>Name of the insurance : <span>Nill</span></li>';
								}
								if(!empty($userTimeslotlist->insuranceid_card)){ 
									$str .= '<li>Insurance Card No. : <span>'.$userTimeslotlist->insuranceid_card.'</span></li>';
								}
								if(!empty($userTimeslotlist->insurance_comp_address)){ 
									$str .= '<li>Address of insurance company : <span>'.$userTimeslotlist->insurance_comp_address.'</span></li>';
								}
								if(!empty($userTimeslotlist->group_insurance)){ 
									$str .= '<li>Group No. of insurance card : <span>'.$userTimeslotlist->group_insurance.'</span></li>';
								}
								
							}else{
								$str .= '<li>Insurance Type : <span>Non Insurance</span></li>';
								if($userTimeslotlist->booking_pay_type=='P'){
									$str .= '<li>Payment Method : <span>Paypal</span></li>';	
								}else if($userTimeslotlist->booking_pay_type=='B'){
									$str .= '<li>Payment Method : <span>Blue Pay</span></li>';
								}
								if(!empty($userTimeslotlist->paymentHistories[0]['payment_status'])){
									if(!empty($userTimeslotlist->paymentHistories[0]['payment_status'])){ 
										$payment_status = @$userTimeslotlist->paymentHistories[0]['payment_status'];
									}else{
										$payment_status = 'Nill';
									}
									$str .= '<li>Payment Status : <span>'.$payment_status.'</span></li>';
								}	
							}
					...
					<div class="col-md-12" id="appointment_reasonid" style="display:none;">
						<select name="appointment_reason_id" id="appointment_reason_id">';
						$reasonslist = AppointmentReasons::find()->where(['status'=>'1'])->asArray()->all();
							if(count($reasonslist)>0){
								$str .= '<option value="">---Select Reason---</option>';
								foreach($reasonslist as $value){
									$str .= '<option value="'.$value['id'].'" '.$selected.'>'.$value['name'].'</option>';
								}
							}
						$str .= '</select>
					...
					<div class="col-md-12">
							<div id="shiftpatient"></div>
							<div id="appointment_savebtn" style="display:none;"><input type="button" class="shiftbuttons" name="shiftButton" id="shiftButton" value="Save" onclick=\'shiftProvidersappoointment("'.$userTimeslotlist->booking_date.'", "'.$userTimeslotlist->booking_time.'", "'.$userTimeslotlist->provider->id.'", "'.$userTimeslotlist->id.'")\'></div>
					</div>';
					echo $str;
		}
	}
	public function actionShiftproviderappointment(){
		$statusid = Yii::$app->request->post('statusid');
		$slotid = Yii::$app->request->post('slotid');
		$providerid = Yii::$app->request->post('providerid');
		$order_status = Yii::$app->request->post('order_status');
		if(Yii::$app->request->post()!=null){
			return $statusid."#".$order_status;
		}
	}
	
	public function actionChangeappointmentstatus(){
		$statusid = Yii::$app->request->post('statusid');
		$bookingslotid = Yii::$app->request->post('slotid');
		$providerid = Yii::$app->request->post('providerid');
		$order_status = Yii::$app->request->post('order_status');
		if(Yii::$app->request->post()!=null){
					$adminEmailid = User::find()->where(['status'=>1, 'id'=>1])->one();
					$userTimeslotlist = UserTimeslotBooking::find()->select('provider_id, id, user_id, fullname, email, booking_date, booking_time')->with('user')->where(['id'=>$bookingslotid])->andWhere(['status'=>'1'])->one();
					
					#--CHANGE THE STATUS---#
					$bookingModel = UserTimeslotBooking::find()->where(['id'=>$bookingslotid])->andWhere(['status'=>'1'])->one();
					if(count($bookingModel)>0){
						$bookingModel->booking_status_id = $statusid;
						$bookingModel->save(false);
					}
					#---END---#

					$messageBody = 'Hi <b>'. $userTimeslotlist->fullname.' ,</b>
					<p>Your appointment has been “'.$order_status.'” with <b>HealthCare800.</b></p>
					<p>Kindly find your <b>“Appointment Details”</b> below :- </p>
					<p>Booking Date : '.date("M, d Y", strtotime($userTimeslotlist->booking_date)).'</p>
					<p>Booking Time : '.date('G:i A', strtotime($userTimeslotlist->booking_time)).'</p>
					<p>Booking Status : <b>'.$order_status.'</b></p>
					<p><b>Thanks & Regards,</b><br>HealthCare800 Team,<br><a href="http://www.healthcare800.com" target="_blank">www.healthcare800.com</a></p>';
					
					$sendemail1 = \Yii::$app->mailer->compose()
						->setTo($userTimeslotlist->email)
						->setFrom([$adminEmailid->email => 'HealthCare800'])
						->setSubject('Your appointment has been “'.$order_status.'” successfully.')
						->setHtmlBody($messageBody)
						->send();
					if($sendemail1){
						echo "sent";
   ...
   public function actionShifttimeslotappointment(){
		$providerid = Yii::$app->request->post('provider_id');
		$time_slot = Yii::$app->request->post('time_slot');
		$slot_date = Yii::$app->request->post('slot_date');
		$timeslotid = Yii::$app->request->post('timeSlotid');
		if(!empty($time_slot) && (!empty($slot_date))){
			echo ''.date("D, M d", strtotime($slot_date))." - ".date('G:i A', strtotime($time_slot))." <div class='clear'></div><input type=\"button\" class=\"shiftbuttons\" name=\"shiftButton\" id=\"shiftButton\" value=\"Save\" onclick=\"shiftProvidersappoointment('".$slot_date."', '".$time_slot."', '".$providerid."', '".$timeslotid."')\"><div id='shiftmsg'></div>";
		}
	}
	
	public function actionShiftappointment(){
		
		$providerid = Yii::$app->request->post('provider_id');
		$booking_time = Yii::$app->request->post('timeslot');
		$booking_date = Yii::$app->request->post('slotdate');
		$timeslotid = Yii::$app->request->post('time_slotid');
		$appointmentreasonid = Yii::$app->request->post('appointment_reasonid');
		$order_status = Yii::$app->request->post('order_status');
		$orderstatusid = Yii::$app->request->post('orderstatusid');

		if(!empty($booking_time) && (!empty($booking_date)) && (!empty($timeslotid))){
			$bookingModel = UserTimeslotBooking::find()->select('provider_id, id, user_id, fullname, email, booking_date, booking_time, appointment_reason_id')->where(['id'=>$timeslotid])->one();
			$bookingModel->booking_date = $booking_date;
			$bookingModel->booking_time = $booking_time;
			$bookingModel->booking_status_id = $orderstatusid;
			$bookingModel->appointment_reason_id = $appointmentreasonid;
			if($bookingModel->save(false)){
				echo date("D, M d", strtotime($booking_date)).' - '.date('G:i A', strtotime($booking_time));
				$adminEmailid = User::find()->where(['status'=>1, 'id'=>1])->one();
				
				#--CHANGE THE STATUS---#
				$appointmentreason = AppointmentReasons::find()->where(['id'=>$appointmentreasonid])->andWhere(['status'=>'1'])->one();
				$bookingModel = UserTimeslotBooking::find()->where(['id'=>$timeslotid])->andWhere(['status'=>'1'])->one();
				if(count($bookingModel)>0){
					$bookingModel->booking_status_id = $orderstatusid;
					$bookingModel->save(false);
				}
				...
				$messageBody = 'Hi <b>'. $bookingModel->fullname.' ,</b>
					<p>Your appointment has been Shifted with <b>HealthCare800.</b></p>
					<p>Kindly find your <b>“Appointment Details”</b> below :- </p>
					<p>Booking Date : '.date("M, d Y", strtotime($booking_date)).'</p>
					<p>Booking Time : '.date('G:i A', strtotime($booking_time)).'</p>
					<p>Booking Status : <b>'.$order_status.'</b></p>
					<p>Reasons For Reschedule or Cancel Appointment : <b>'.$appointmentreason->name.'</b></p>
					<p><b>Thanks & Regards,</b><br>HealthCare800 Team,<br><a href="http://www.healthcare800.com" target="_blank">www.healthcare800.com</a></p>';
					
					$sendemail1 = \Yii::$app->mailer->compose()
						->setTo($bookingModel->email)
						->setFrom([$adminEmailid->email => 'HealthCare800'])
						->setSubject('Your appointment has been “Shifted” successfully.')
						->setHtmlBody($messageBody)
						->send();
					if($sendemail1){
						echo "sent";
